...
```{r}
#Plotting a subset of the data with more zoomed in axes
tidy_df_sub = filter(tidy_df, maternal_trees == c(25, 50, 100))
preds_sub = filter(preds, maternal_trees == c(25, 50, 100))

pdf("FigureS0.pdf", height = 10, width = 8)
ggplot(data=preds_sub) +
    ...
    facet_wrap(vars(maternal_trees), labeller = labeller(maternal_trees = mat_tree_labs), ncol=1) +
